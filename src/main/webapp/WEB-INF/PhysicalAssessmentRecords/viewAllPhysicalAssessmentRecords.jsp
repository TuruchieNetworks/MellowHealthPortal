...
<body class="container-fluid p-8" style="
	...
	<h1 style="text-align:center;border-bottom: 2px solid chocolate;color: brown; font-family:fantasy;background:rgba(10.2, 3.3, 3.6, 0.9);margin-top:5px;border-radius:5%;">
		<a href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id }" style=" margin: 0 15px 0 0; display:block; padding: 10px;color: khaki;text-decoration:none;font-size:26px;">
			<c:out value="Welcome To The Mellow Physical Assessment Dashboard ${currentDateTime} ${loggedInPatient.patientFirstName}!"/>
		</a>
	</h1>
	<table class="table table-dark" style="text-align:center;border-radius:5%;">
		...
		<div class="btn btn-primary generic-display-container" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:space-between;text-align:center;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;width:100%;">
		    <form action="/mellowHealth/diagnosis/physicalAssessments" class="btn btn-outline-primary" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; padding:5px;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;margin:5px;width:100%;">
		...
		<c:forEach items="${physicalAssessmentList}" var="assessment" varStatus="status">
		    <div class="column-card" style="text-decoration:none; color:aqua;background: rgba(13, 0.64, 0.1, 0.9);">
		            <div style="text-decoration:none; color:aqua"class="table-dark column-card">
		                <div>
		                	<a href="/mellowHealth/diagnosis/physicalAssessments/${assessment.id}"style="text-decoration:none; color:aqua">
		                		<c:out value="Assessment: ${assessment.id}" />
		                	</a>
		                </div>
		                <div>
		                	<a href="/mellowHealth/diagnosis/physicalAssessments/${assessment.id}"style="text-decoration:none; color:aqua">
		                ...
		                <div>
		                	<a href="/mellowHealth/diagnosis/physicalAssessments/${assessment.id}"style="text-decoration:none; color:silk">
		                		<c:out value="Lungs Description: ${assessment.lungsDescription}"/>
		                	</a>
		                </div>
		                <div>
		                	<a href="/mellowHealth/diagnosis/physicalAssessments/${assessment.id}"style="text-decoration:none; 
				                <c:choose>
	               				...
	               				<c:out value="Heart State: ${assessment.heartDescription}"/>
	               			</a>
	               		</div>
		                <div>
		                	<a href="/mellowHealth/diagnosis/physicalAssessments/${assessment.id}"style="text-decoration:none; 
				        ...
				        <div style=" 	
			        		<c:choose>
				             ...
				             " class="d-flex justify-content-around card-column">
								<form action="/mellowHealth/diagnosis/physicalAssessments/editPhysicalAssessment/${assessment.id}" method="get" style="">
								    <input type="hidden" name="_method" value="edit">
								    <input class ="btn btn-outline-warning" type="submit" value="Edit Record" style=" margin:5px; width: 100%;" >
								</form>
								<form action="/mellowHealth/diagnosis/physicalAssessments/deletePhysicalAssessment/${assessment.id}" method="post">
								    <input type="hidden" name="_method" value="delete">
									<input class ="btn btn-outline-danger" type="submit" value="Delete Record" style=" margin:5px; width: 100%;">
								</form>
						</div>
		 				</c:when>
						<c:otherwise>
				   ...
				   ">	
			        <div class="btn btn-outline-warning column-card" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center;margin:5px; padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);">
						<form action="/mellowHealth/insuranceRecords/${patientCase.insuranceInformation.id}" method="get" style="width:100%;">
							<input type="hidden" name="_method" value="get">
							<input class ="btn btn-outline-warning" type="submit" value="View Insurance Record" style="padding: 10px;width:100%;" >
						</form>
					</div>
			        <div class="btn btn-outline-warning column-card" style="background:rgba(1.33, 0.64, 0.60, 0.9);">
						<h1 style="width:100%;">
						 	<a style="background:rgba(8, 8, 10, 0.9);width:100%;" href="/mellowHealth/patientsPortal/${loggedInPatient.id}" class="btn btn-outline-success">
						 		<c:out value="Mellow Patient Since ${loggedInPatientCreatedAt}"/>
							</a>
					 	</h1>
				 	</div>
				</div>
				    <div class="form-group column-card" style="margin:10px 0;
				    <c:choose>
				        <c:when test="${loggedInPatient.insuranceRecords.size() <= 2}">
						     	 color: rgb(412, 580, 515); background: rgba(10.531, 10.64, 0.36, 0.9);
				        </c:when>
				   ...
				   ">
			        <div class="btn btn-outline-warning column-card" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center;background:rgba(1.33, 0.64, 0.60, 0.9);">
						<h1 style="width:100%;margin:5px;">
						 	<a style="background:rgba(8, 8, 10, 0.9);width:100%;" href="/mellowHealth/patientsPortal/${loggedInPatient.id}" class="btn btn-outline-success">
						 		<c:out value="View Diagnostic Record for ${searchedPatientCaseCreatedAt}'s Visit"/>
							</a>
					 	</h1>
				 	</div>	

			        <div class="btn btn-outline-danger column-card" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center;margin:5px; padding:2px;background:rgba(1.33, 0.64, 0.60, 0.9);">
		...
		<div class="btn btn-primary" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:space-between;text-align:center;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;margin:5px 0;width:100%;">
			...
			<c:if test="${not empty searchedPatientCase}">
			    <p class="btn btn-outline-primary form-control" style="color:rgba(311, 31, 321, 0.9);background:rgba(11, 0.31, 1, 0.9);">
					    ...
					    <c:out value="${searchedPatientCase[0].patient.patientFirstName} ${searchedPatientCase[0].patient.patientLastName} Date Of Birth: ${searchedPatientCase[0].patient.dateOfBirth} ${searchedPatientCase[0].patient.patientCases.size()} Patient Cases Today ${dayCurrentDateTime}"/>
					</a>
			    </p>
			</c:if>
		</div>
		...
		<div class="form-group column-card" style="
			...
			<h1 style=" margin:5px; width: 100%">
				<a href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id}" class="btn btn-outline-primary" style=" margin:5px; width: 100%; display:block; padding: 10px;background:rgba(0.1, 0.3, 20, 0.9);">
					<c:out value="${loggedInPatient.patientFirstName} ${loggedInPatient.patientLastName} Records"/>
				</a>
			</h1>
			<h1 style=" margin:5px; width: 100%">
				<a href="/mellowHealth/patientsPortal/logout" class="btn btn-outline-warning" style="width: 100%; display:block; padding: 10px; margin:5px;background:rgba(0.1, 0.3, 10, 0.9);">
					<c:out value="LOGOUT HERE!"/>
				</a>
			</h1>
			<form action="/mellowHealth/insuranceRecords" method="get" style=" margin:5px;">
				<input type="submit" value="View All Insurance Records!" class="btn btn-outline-primary"style="margin:5px; width:100%;background:rgba(10.1, 0.3, 10, 0.9);"/>
			</form>
		</div>	
