	...
	<div class="container-fluid p-6" style=" text-align:center;">
   	<c:choose>
        <c:when test="${empty oneSearchedPatientCase}">
			<a style="width: 100%; display:block; padding:0px;color:brown;text-decoration:none;" href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id }">
				<h1 style="color:azure;border-bottom:2px solid aliceblue;background:rgba(2.11, 24.5, 121, 0.9);border-radius:5%;font-weight:bold;font-size:22px;padding:10px;font-family:cursive;text-align:center;">
					<c:out value="Make Changes On Your Physical Assessment Record Today ${currentDateTime}"/> <c:out value="${loggedInPatient.patientFirstName}"/> <c:out value="${loggedInPatient.patientLastName}!"/>
				</h1>
			</a>
        </c:when>
		<c:otherwise>
			<a style="width: 100%; display:block; padding:0px;color:brown;text-decoration:none;" href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id }">
				<h1 style="color:azure;border-bottom:2px solid aliceblue;background:rgba(2.11, 24.5, 121, 0.9);border-radius:5%;font-weight:bold;font-size:22px;padding:10px;font-family:cursive;text-align:center;">
					<c:out value="Make Changes On Your Physical Assessment Record Today ${currentDateTime}"/> <c:out value="${oneSearchedPatientCase.patient.patientFirstName}"/> <c:out value="${oneSearchedPatientCase.patient.patientLastName}!"/>
				</h1>
			</a>
		</c:otherwise>
	</c:choose>
			...
			<div class ="btn btn-primary"  style="width:100%;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;">
				<form action="/mellowHealth/diagnosis/physicalAssessments/editPhysicalAssessment/${physicalAssessment.id}" class ="btn btn-primary"  style="width:100%;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; padding:5px;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;">
		...
		<div class="row">
			...
			<div class="col">
				<form:form action="/mellowHealth/diagnosis/process/physicalAssessments/editPhysicalAssessment/${physicalAssessment.id}" method="PATCH" modelAttribute="physicalAssessment">
				...
				<!--form:errors path="*" cssClass="text-danger"/!-->
					...
					<input type="submit" value="Edit Physical Assessment Record" class="btn btn-success" style="margin: 10px 0; width: 100%; padding: 10px;"/>
				</form:form>
