...
<body class="container-fluid p-8" style="
	...
	<table class="table table-dark" style="text-align:center;border-radius:5%;">
		...
		<tbody>
		<c:forEach items="${loggedInPatient.pastMedicalHistories}" var="medicalRecord" varStatus="status">
		    <c:if test="${status.index < lengthOfMedicalConditionMap.size() and status.index < pastMedicalHistoryDayCreatedAtList.size()}">
	        <!-- Access the current element from each collection using status.index -->
	        <c:set var="pastMedicalHistory" value="${medicalRecord}" />
	        <c:set var="lengthOfCondition" value="${lengthOfMedicalConditionMap[pastMedicalHistory]}" />
	        <c:set var="pastMedicalHistoryCreatedAt" value="${pastMedicalHistoryDayCreatedAtList[status.index]}" />
		                ...
		                <td>
		                	<a href="/mellowHealth/pastMedicalRecords/${medicalRecord.id}"style="text-decoration:none; 
				                <c:choose>
	               				...
	               				<c:out value="${lengthOfCondition} Years ${pastMedicalHistoryCreatedAt} Record"/>
	               			</a>
	               		</td>
		           		<c:choose>
						...
						<td class="table-dark d-flex justify-content-around">
							<a href="/mellowHealth/pastMedicalRecords/${pastMedicalHistory.id}" style="text-decoration:none; 
								<c:choose>
		               			...
		               			<c:out value="${medicalRecord.medicalCondition}: Coverage Period ${medicalRecord.startDate} - ${lengthOfCondition} years"/>
		               		</a>
		               	</td>
						</c:otherwise>
            			</c:choose>
