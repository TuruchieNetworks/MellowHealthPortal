...
<body class="container-fluid p-8" style="
	...
	<h1 style="text-align:center;border-bottom: 2px solid chocolate;color: brown; font-family:fantasy;background:rgba(10.2, 3.3, 3.6, 0.9);margin-top:5px;border-radius:5%;">
		<a href="/mellowHealth/insuranceRecords/${insuranceReport.id}" style=" margin: 0 15px 0 0; display:block; padding: 10px;color: khaki;text-decoration:none;font-size:28px;">
			<c:out value="Welcome To Your ${insuranceReport.providerName} Insurance Dashboard ${loggedInPatient.patientFirstName} ${currentDateTime}!"/>
		</a>
	</h1>
	<table class="table table-dark" style="text-align:center;border-radius:5%;">
		  ...
		  <tbody>
		          <tr style="text-decoration:none; color:aqua"class="table-dark">
		                ...
		                <td>
		                	<a href="/mellowHealth/pastMedicalRecords/${medicalRecord.id}"style="text-decoration:none; color:aqua">
		                		<c:out value="${medicalRecord.medicalCondition}"/>
		                	</a>
		                </td>
		                <td>
		                	<a href="/mellowHealth/pastMedicalRecords/${medicalRecord.id}"style="text-decoration:none; color:silk">
		                		<c:out value="${medicalRecord.treatmentPlan}"/>
		                	</a>
		                </td>
		                <td>
		                	<a href="/mellowHealth/pastMedicalRecords/${medicalRecord.id}"style="text-decoration:none; 
				                <c:choose>
	               				...
	               				<c:out value="${medicalRecord.startDate}"/>
	               			</a>
	               		</td>
		                <td>
		                	<a href="/mellowHealth/pastMedicalRecords/${medicalRecord.id}"style="text-decoration:none; 
				                <c:choose>
	               				...
	               				<c:out value="${loggedInPatientLengthOfConditionYears} Years: ${dayCreatedAt} Record"/>
	               			</a>
	               		</td>
		           		<c:choose>
						<c:when test="${medicalRecord.patient.id == loggedInPatient.id}">
				        <td style=" 	
			        		<c:choose>
					            <c:when test="${loggedInPatient.insuranceRecords.size() < 1}">
				             ...
				             " class="d-flex justify-content-around">
								<form action="/mellowHealth/pastMedicalRecords/editPastMedicalRecord/${medicalRecord.id}" method="get" style="margin:0 10px 0 0; ">
								...
								<form action="/mellowHealth/pastMedicalRecords/deletePastMedicalRecord/${medicalRecord.id}" method="post">
						...
						<td class="table-dark d-flex justify-content-around">
							<a href="/mellowHealth/pastMedicalRecords/${pastMedicalHistory.id}" style="text-decoration:none; 
								<c:choose>
				                    <c:when test="${loggedInPatient.insuranceRecords.size() < 1}">
		               			...
		               			<c:out value="${medicalRecord.medicalCondition}: Coverage Period ${medicalRecord.startDate} - ${createdAt} Record"/>
		               		</a>
		               	</td>
						</c:otherwise>
            			</c:choose>
	...
	<div class="form-group"style="
	    ...
	    display:flex; justify-content:space-between;align-items:center; border-radius:5%;padding:5px;margin:10px 0;">
	 	<h1 style="width: 100%">
	 		<a class="btn btn-outline-success" style=" margin:10px 0 ; width: 100%; display:block; padding: 15px;background: rgba(13, 0.123, 0.160, 0.9);"  href="/mellowHealth/pastMedicalRecords" class="btn btn-success">
	 		<c:choose>
		        <c:when test="${loggedInPatient.pastMedicalHistories.size() < 2}">
	 				<c:out value="VIEW ALL ${loggedInPatient.pastMedicalHistories.size()} PAST MEDICAL RECORD TODAY: ${currentDateTime}"/>
		        </c:when>
		        <c:otherwise>
	 				<c:out value="VIEW ALL ${loggedInPatient.pastMedicalHistories.size()} PAST MEDICAL RECORDS: ${currentDateTime}"/>
		        </c:otherwise>
		    </c:choose>
	 		</a>
	 	</h1>
	...
	<div class="form-group"style="
		...
		<h1 style=" margin:5px; width: 100%">
			<a href="/mellowHealth/pastMedicalRecords/newPastMedicalRecord" class="btn btn-outline-success" style=" margin: 5px; width: 100%; display:block; padding: 10px;background: rgba(2, 10.13, 0.160, 0.9);">
