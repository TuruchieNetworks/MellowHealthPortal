...
<body class="container-fluid p-8" style="
    ...
    <table class="table table-dark text-center" style="border-radius: 5%;">
		...
		<c:forEach items="${searchedPatientCaseCoagulationList}" var="searchedCoagulation">
		<c:if test="${searchedCoagulation.patient.id == loggedInPatient.id}">
        <div class="column-card">
    		<div class="inner-column-card"  style="padding:10px;">
	        	<p class="btn btn-outline-success" style="border-radius:5%;">
	        		<c:out value="${searchedPatientCaseCoagulationRecordCreatedAt} Coagulation Report ID: ${searchedCoagulation.id}"/>
			        <a class="column-card" href="/mellowHealth/coagulationRecords/${searchedCoagulation.id}"  style="text-decoration:none; 
		         	<c:choose>
	                   <c:when test="${createdAt.contains('Sat') || createdAt.contains('Sun')}">
			                color: rgb(11, 180, 355);
	                   </c:when>
	            	...
	            	<c:out value="${searchedCoagulation.patient.patientFirstName} ${searchedCoagulation.patient.patientLastName}: Mellow Patient Since ${onePatientCreatedAt}"/>
	            	<div class="column-card btn btn-outline-primary" style="justify-content:center;align-items:center;text-align:center;margin:5px 0; padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);">
			            <c:out value="Diagnostic WorkUp: ${oneSearchedPatientCaseDiagnosticRecord.diagnosticWorkUp} Albumin Levels: ${searchedCoagulation.albumin} g/dL"/>
			            <c:out value="Prothrombin Time: ${searchedCoagulation.prothrombinTime}seconds- Partial Thromboplasting: ${searchedCoagulation.partialThromboplastinTime} Seconds"/>
		           		<div class="column-card btn btn-outline-primary" style="background:rgba(1.33, 10.64, 0.60, 0.9);">
		            		<c:out value="Objective Findings: ${searchedPatientCaseDiagnosticRecord.objectiveFindings} Physical Exam Findings: ${searchedPatientCaseDiagnosticRecord.physicalExamFindings}"/>
		            		<p class="btn btn-outline-success" style="margin:5px 0;">
			            		<c:out value="Coagulation: ${searchedCoagulation.coagulation} seconds"/>
			            		<c:out value="Fibrinogen: ${searchedCoagulation.fibrinogen} mg/dL"/>
			            	</p>
			            </div>
			            <p>
			            	<c:out value="Contact Details: ${searchedCoagulation.patient.patientAddresses[0].phoneNumber}"/>
			            	<c:out value="Email Address: ${searchedCoagulation.patient.email}"/>
				            <c:out value="Insurance Provider: ${mostRecentInsuranceReport.providerName}"/>
				           	<c:out value="Coverage Details: ${mostRecentInsuranceReport.expirationDate}"/>
				       
			            </p>
		            </div>
					<div class="btn btn-outline-success column-card" style="padding:5px;">
						<div class="btn btn-outline-success inner-generic-display-container" style="padding:5px;background:rgba(1.33, 0.64, 0.60, 0.9);background:rgba(13.33, 0.64, 0.60, 0.9);">
		        			<form action="/mellowHealth/patientsPortal/patients/${searchedCoagulation.patient.id}" method="get" style="margin:5px;width:100%;">
				    		...
				    		<form action="/mellowHealth/coagulationRecords/editCoagulationRecord/${searchedCoagulation.id}" method="get" style="margin:5px;width:100%;">
							    <input type="hidden" name="_method" value="edit">
							    <input class ="btn btn-outline-warning" type="submit" value="Update Coagulation Record" style="width:100%;background:rgba(13.33, 0.64, 0.60, 0.9);" >
							</form>
		        			<form action="/mellowHealth/coagulationRecords/deleteCoagulationRecord/${searchedCoagulation.id}" method="post" style="margin:5px;width:100%;">
							    <input type="hidden" name="_method" value="delete">
							    <input class ="btn btn-outline-danger" type="submit" value="Delete Coagulation Record" style="width:100%;background:rgba(13.33, 0.64, 0.60, 0.9);" >
							</form>
			        	</div>
		       		</div>
					</a>
		...
		<div class="form-group column-card" style="padding:10px;
	    	<c:choose>
	            <c:when test="${patientCase.patient.gender == 'Male' || patientCase.patient.race == 'Black'}">
			         color: rgb(412, 580, 515);background: rgba(13, 0.64, 60, 0.9); 
	            </c:when>
				 	...
				 	<a href="/mellowHealth/patientsPortal/patients" class="btn btn-warning" style="
			            ...
			            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
			             ...
			             width: 100%; display:block; padding: 10px">
			           	 <c:out value="Patients Portal Access!"/>
			          </a>
			      </h1>
				  <form action="/mellowHealth/patientsPortal/patients/${patientCase.patient.id}" method="get" style="width:100%; background: rgba(0.531, 0.64, 16, 0.9);">
				     <input type="submit" value="${loggedInPatient.patientCases.size()}: Patient Cases- ${loggedInPatient.physiciansPatients.size()}: Private Patients!" class="btn btn-outline-primary" style="background: rgba(0.531, 10.64, 16, 0.9);
			    		<c:choose>
			            <c:when test="${patientCase.patient.gender == 'Male' || patientCase.patient.race == 'Black'}">
					         color: rgb(412, 580, 515);background: rgba(13, 64, 60, 0.9); 
			            </c:when>
		...
		<div class="form-group"style="
		 	...
		 	<h1 style=" margin: 10px; width: 100%">
		 		<a href="/mellowHealth/hospitalDashboard/patientCases/newPatientCase" class="btn btn-warning" style=" margin: 0 15px 0 0; width: 100%; display:block; padding: 10px;font-weight:bold;">
		 			<c:out value="SCHEDULE NEW PHYSICIAN VISIT!"/>
		 		</a>
		 	</h1>		
			<h1 style="width: 100%;margin:10px; ">
				<a style=" width: 100%; display:block; padding: 10px" href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id}" class="btn btn-primary">
					<c:out value="You have ${loggedInPatient.patientCases.size()} Patients In Care${loggedInPatient.patientFirstName} ${loggedInPatient.patientLastName}!"/>
				</a>
			</h1>	
		</div>
		<div class="form-group"style="
