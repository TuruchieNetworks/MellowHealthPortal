...
<body class="container-fluid p-8" style="
	...
	<table class="table table-dark" style="text-align:center;border-radius:5%;">
		...
		<div class="btn btn-primary generic-display-container" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:space-between;text-align:center;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;width:100%;">
		    <form action="/mellowHealth/incidentReports" class="btn btn-outline-primary" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; padding:5px;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;margin:5px;width:100%;">
		...
		<c:forEach items="${searchedIncidentReportsList}" var="incidence" varStatus="status">
		<c:if test="${incidence.patient.id == loggedInPatient.id}">
		<div class="main-container-column btn btn-outline-success" style="width:100%;display:flex;align-items:center;flex-direction:column;padding:12px;background:rgba(1.33, 10.64, 0.60, 0.9);">
			<div class="btn btn-outline-primary column-card" style="display:flex;justify-content:center;align-items:center;text-align:center;padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);width:100%;">
			     <div class="inner-column-card btn btn-outline-success" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:5px; padding:5px;">
			      	<c:out value="Incidence ID: ${incidence.id} Incidence Onset: ${incidence.onset} Day: ${incidentOnsetDaysHistory} Progress"/>
			         <a class="inner-column-card btn btn-outline-success" href="/mellowHealth/incidentReports/${incidence.id}"style="text-decoration:none; color:aqua;background:rgba(1.33, 0.64, 0.60, 0.9);">
			           <c:out value="${incidence.timeOfOccurrence} Event: ${incidence.event} ${incidentReportDayCreatedAt} Record"/>
			         </a>
			      </div>
				  <div class="column-card btn btn-outline-danger" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:5px; padding:5px;background:rgba(1.33, 0.64, 0.60, 0.9);">
			      	<c:out value="Respiratory Rate: ${incidence.patientCase.patientVitalRecords[0].respiratoryRate} Pulse Rate: ${incidence.patientCase.patientVitalRecords[0].pulseRate}"/>
			         <a href="/mellowHealth/incidentReports/${incidence.id}" class="inner-column-card 
					    <c:choose>
					        <c:when test="${incidence.patientCase.patientVitalRecords[0].heartRate < 70}">
					            btn btn-outline-primary
					        </c:when>
					        <c:when test="${incidence.patientCase.patientVitalRecords[0].heartRate > 70 && incidence.patientCase.patientVitalRecords[0].heartRate < 80}">
					    ...
					    <c:out value="Relieving Agent: ${incidence.relievingAgentTaken}- ${incidence.onset} ${incidentOnsetDaysHistory} Days History"/>
					</a>
			   ...
			   <c:choose>
			    ...
			    </c:choose>" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;padding:10px;">
		          <a href="/mellowHealth/incidentReports/${incidence.id}" style="text-decoration:none; 
					  <c:choose>
		              ...
		              <p class="inner-column-card">
			              <c:out value="Current Condition Status: ${incidence.conditionStatus} Recorded Blood Pressure: ${incidence.patientCase.patientVitalRecords[0].systolicBloodPressure}/${incidence.patientCase.patientVitalRecords[0].diastolicBloodPressure}mmHg "/>
		              </p>
		              <c:out value="Pulse Rate: ${incidence.patientCase.patientVitalRecords[0].pulseRate}bpm Respiratory Rate: ${incidence.patientCase.patientVitalRecords[0].respiratoryRate}bpm "/>
	             </a>
		         <a href="/mellowHealth/incidentReports/${incidence.id}" style="text-decoration:none; 
					 <c:choose>
					     <c:when test="${loggedInPatient.insuranceRecords.size() < 1}">
		               ...
		               <div style="background:rgba(1.33, 0.64, 0.60, 0.9);align-items:center;"class="
					     <c:if test="${bmi <= 18.4}">
		               ...
		               ">
			            <p class="inner-column-card
							     ...
							     <c:if test="${bmi >= 25.0 && bmi <= 39.9}">
									  btn btn-outline-danger
							     </c:if>" style="background:rgba(11.33, 0.64, 0.60, 0.9);">
			               	<c:out value="Weight: ${incidence.patientCase.patientVitalRecords[0].weight}Lb"/>
			               	<c:out value="Height: ${incidence.patientCase.patientVitalRecords[0].height}cm"/>
               	...
               	<c:choose>
				     <c:when test="${incidence.patientCase.patientVitalRecords[0].abdominalPainAssessments.size() <=1}">
				           <c:out value="${incidence.patientCase.patientVitalRecords[0].abdominalPainAssessments.size()} Abdominal Pain Assessment"/>
				     </c:when>
				     <c:otherwise>
			           <c:out value="${incidence.patientCase.patientVitalRecords[0].abdominalPainAssessments.size()} Abdominal Pain Assessments"/>
				     </c:otherwise>
			     </c:choose>
               	<c:choose>
				     <c:when test="${incidence.patientCase.patientVitalRecords[0].nauseaVomitAssessments.size() <=1}">
				           <c:out value="${incidence.patientCase.patientVitalRecords[0].nauseaVomitAssessments.size()} Nausea Vomit Assessment"/>
				     </c:when>
				     <c:otherwise>
			           <c:out value="${incidence.patientCase.patientVitalRecords[0].nauseaVomitAssessments.size()} Nausea Vomit Assessments"/>
				     </c:otherwise>
			     </c:choose>
               	<c:choose>
				     <c:when test="${incidence.patientCase.patientVitalRecords[0].nauseaVomitAssessments.size() <=1}">
				           <c:out value="${incidence.patientCase.patientVitalRecords[0].patientVisitEvaluations.size()} Patient Visit Evaluation"/>
				     </c:when>
				     <c:otherwise>
			           <c:out value="${incidence.patientCase.patientVitalRecords[0].patientVisitEvaluations.size()} Patient Visit Evaluations"/>
				     </c:otherwise>
			     </c:choose>
               	...
               	<div class="column-card" style="background:rgba(50.9, 10.1, 11, 0.9);padding:px;">				
               		<form action="/mellowHealth/incidentReports" method="get" style="width:100%;margin:5px;">
						...
						<input class ="
					     ...
					     <c:if test="${bmi >= 25.0 && bmi <= 39.9}">
							  btn btn-outline-danger
					     </c:if>" type="submit" value="Relieving Agent: ${incidence.relievingAgentTaken} ${incidence.timeOfOccurrence}" style="width:100%;padding:10px;">
					</form>				
               		<form action="/mellowHealth/incidentReports" method="get" style="width:100%;margin:5px;" class="">
					    <input type="hidden" name="_method" value="get">
						<input class ="btn btn-outline-danger" type="submit" value="Aggravating Agent: ${incidence.aggravatingAgentTaken} ${incidence.onset}" style="width:100%;padding:10px;background:rgba(50.9, 10.1, 1, 0.9);">
					</form>
				</div>	 
		               ...
		               <c:out value="PatientCase Progress: Day ${patientCaseAccountDaysHistory}"/>
	             	   <p>
	             	   		<c:out value="Presenting Chief Complaint: ${incidence.patientCase.patientVitalRecords[0].patientCase.chiefComplaint}: ${searchedPatientVitalCreatedAt} Record"/>
	             	   	</p>
	            	</div>
				 <div class="inner-column-card btn btn-outline-primary" style="width:100%;display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:5px;padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);">
           		 <c:choose>
					<c:when test="${incidence.patientCase.patientVitalRecords[0].patient.id == loggedInPatient.id}">
				        <div class="column-card" style="width:100%;	
			        		<c:choose>
			 				...
			 				<div class="btn btn-outline-primary unwrapped-inner-column-card" style="border-radius:5%;background:rgba(1.33, 0.64, 0.60, 0.9);padding:10px;">
								<form action="/mellowHealth/incidentReports/editIncidentReport/${incidence.id}" method="get" style="margin:5px;width:100%;">
								    <input type="hidden" name="_method" value="edit">
								    <input class ="btn btn-outline-warning" type="submit" value="Edit Incident Report" style="width:100%;padding:10px" >
								</form>
								<form action="/mellowHealth/incidentReports/deleteIncidentReport/${incidence.id}" method="post" style="margin:5px;width:100%;">
						...
						<div class="table-dark d-flex justify-content-around">
							<a href="/mellowHealth/incidentReports/${incidences.id}" style="text-decoration:none; 
								<c:choose>
		               			...
		               			<c:out value="${incidence.patientCase.patientVitalRecords[0].patientCase.insuranceInformation.providerName}: Coverage Period ${incidence.patientCase.patientVitalRecords[0].patientCase.insuranceInformation.insuranceId} - ${lengthOfCoverage} years"/>
		               		</a>
		               	</div>
						</c:otherwise>
            			</c:choose>
	    ...
	    <c:choose>
	       ...
	       <c:otherwise>
				<div class="unwrapped-inner-column-card btn btn-outline-primary" style="align-items:center;text-align:center;margin:5px 0; padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);">
			 		...
			 		<a style="width: 100%; display:block; padding: 12px;background: rgba(13, 0.123, 0.160, 0.9);"  href="/mellowHealth/incidentReports/${mostRecentIncidentReport.id}" class="btn btn-outline-warning">
			 			<c:out value="VIEW MOST RECENT INCIDENT REPORT!"/>
			 		</a>
		 		</div>
	 		</c:otherwise>
	      </c:choose>
		        ...
		        <div class="column-card btn btn-outline-primary">
			        ...
			        <div class="inner-generic-tab btn btn-outline-warning" style="">
			            <a class="btn btn-outline-warning" href="/mellowHealth/hospitalDashboard/patientCases/${patientCase.id}" style="text-decoration:none;background:rgba(13.33, 0.64, 0.60, 0.9);
							...
							<form action="/mellowHealth/incidentReports/${incidence.id}" method="get" style="margin:5px;width:100%;">
							    <input type="hidden" name="_method" value="get">
								<input class ="
							     <c:if test="${bmi <= 18.4}">
									  btn btn-outline-primary
