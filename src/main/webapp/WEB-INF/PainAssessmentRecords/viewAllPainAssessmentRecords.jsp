...
<body class="container-fluid p-8" style="
    ...
    <table class="table table-dark text-center" style="border-radius: 5%;">
		...
		<div class="btn btn-primary" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:space-between;text-align:center;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;margin:5px 0;">
		    <form action="/mellowHealth/painAssessments" class="btn btn-outline-primary" style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; padding:5px;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;margin:5px;width:100%;">
        ...
        <div class="column-card">
    		<div class="inner-column-card">
	        	<p class="btn btn-outline-success"  style="margin:10px;">
	        		<c:out value="Address ID: ${address.id}"/>
			        <a class="inner-column-card" href="/mellowHealth/patientAddresses/${address.id}"  style="text-decoration:none; 
		         	<c:choose>
	            	...
	            	<c:out value="${patient.patientFirstName} ${patient.patientLastName}: Mellow Patient Since ${onePatientCreatedAt}"/>
	            	<div class="column-card btn btn-outline-primary" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:5px 0; padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);">
			            <c:out value="Street Name: ${address.street}"/>
		           		<div class="inner-column-card btn btn-outline-primary" style="display:flex;justify-content:space-between;align-items:center;text-align:center;margin:5px; padding:10px;background:rgba(1.33, 10.64, 0.60, 0.9);">
			            	<c:out value="City Info: ${address.city}"/>
			            	<c:out value="${address.state} ${address.zipCode}"/>
			            	<c:out value="Date Of Birth: ${patient.dateOfBirth}"/>
			            </div>
			            <p>
			            	<c:out value="Contact Details: ${address.phoneNumber}"/>
			            	<c:out value="Email Address: ${patient.email}"/>
				...
				<div class="btn btn-outline-success column-card" style="padding:10px;">
					<div class="btn btn-outline-success inner-generic-display-container" style="padding:10px;background:rgba(1.33, 0.64, 0.60, 0.9);background:rgba(13.33, 0.64, 0.60, 0.9);">
	        			<form action="/mellowHealth/patientsPortal/patients/${patient.id}" method="get" style="margin:5px;width:100%;">
			    		...
			    		<form action="/mellowHealth/patientAddresses/editPatientAddress/${address.id}" method="get" style="margin:5px;width:100%;">
						    <input type="hidden" name="_method" value="edit">
						    <input class ="btn btn-outline-warning" type="submit" value="Update Contact Details" style="width:100%;background:rgba(13.33, 0.64, 0.60, 0.9);" >
						</form>
	        			<form action="/mellowHealth/patientAddresses/deletePatientAddress/${address.id}" method="post" style="margin:5px;width:100%;">
						    <input type="hidden" name="_method" value="delete">
						    <input class ="btn btn-outline-danger" type="submit" value="Delete Address Information" style="width:100%;background:rgba(13.33, 0.64, 0.60, 0.9);" >
						</form>
		        	</div>
	       		</div>
			</a>
		...
		<c:forEach items="${allPatientsWithFilter}" var="patient">
		<c:forEach items="${patient.patientCases}" var="patientCase">
		...
		<c:forEach items="${allPatientCasesWithFilter}" var="patientCase">
	        ...
	        <div class="btn btn-outline-warning column-card" style="display:flex;justify-content:space-between;align-items:center;text-align:center;margin:5px; padding:10px; background:rgba(1.33, 0.64, 30.60, 0.9);">
		        <p class="btn btn-outline-primary" style="text-align:center;background:rgba(1.33, 0.64, 30.60, 0.9);">
			      ...
			      <p class="btn btn-outline-success" style="text-align:center;background:rgba(1.33, 0.64, 30.60, 0.9);">
			      		<c:out value="${patientCase.physicalAssessments.size()} Physical Assessment Records"/>
		...
		<div class="form-group"style="
	   	<c:choose>
	            ...
	            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
				 ...
				 <a href="/mellowHealth/hospitalDashboard/patientCases/newPatientCase" class="btn btn-outline-warning" style="width: 100%; display:block; padding: 10px">Schedule A New Visit!</a>
			     </h1>
				 <h1 style="margin: 0px 15px 0 0; width: 100%">
				 	<a href="/mellowHealth/patientsPortal/patients" class="btn btn-outline-primary" style="
			    		<c:choose>
			            <c:when test="${patientCase.patient.gender == 'Male' || patientCase.patient.race == 'Black'}">
					         background: rgba(13, 64, 60, 0.9); 
			            </c:when>
			            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
					         background: rgba(130, 1.14, 160, 0.9); 
			            </c:when>
			            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
					     	 background: rgba(0.531, 0.64, 16, 0.9); 
			             </c:when>
			             <c:otherwise>
			                  background: rgba(0.531, 0.64, 16, 0.9); 
			             </c:otherwise>
				  ...
				  <form action="/mellowHealth/patientsPortal/patients/${patientCase.patient.id}" method="get"
				    style="
			            ...
			            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
					         background: rgba(55, 0.114, 16, 0.9); 
			            </c:when>
			            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
					     	 background: rgba(0.531,64, 16, 0.9); 
			             </c:when>
			             <c:otherwise>
			                  background: rgba(0.531,64, 16, 0.9); 
			             </c:otherwise>
			             </c:choose> ">
			             	<input type="submit" value="${loggedInPatient.patientCases.size()}: Patient Cases- ${loggedInPatient.physiciansPatients.size()}: Private Patients!" class="btn btn-outline-success" style="width:100%; padding: 10px; background: rgba(0.531, 0.64, 1, 0.9); "/>
				  </form>
		</div>
		<div class="form-group"style="
		    	<c:choose>
