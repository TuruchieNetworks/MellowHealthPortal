	...
	<div class="container-fluid p-6" style=" text-align:center;">
		<a style="width: 100%; display:block; padding:0px;color:brown;text-decoration:none;" href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id }">
			<h1 style="color:azure;border-bottom:2px solid aliceblue;background:rgba(2.11, 24.5, 121, 0.9);border-radius:5%;font-weight:bold;font-size:22px;padding:10px;font-family:cursive;text-align:center;">
				<c:out value="Validate Insurance Today ${currentDateTime}"/> <c:out value="${loggedInPatient.patientFirstName}"/> <c:out value="${loggedInPatient.patientLastName}!"/>
			</h1>
		</a>
			<div class ="btn btn-primary"  style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;">
				<form action="/mellowHealth/insuredPatients/newInsuredPatient" class ="btn btn-primary"  style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; padding:5px;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;">
		...
		<div class="row">
			...
			<div class="col">
				<form:form action="/mellowHealth/process/insuredPatients/createNewInsuredPatient" method="POST" modelAttribute="insuredPatients">
				...
				<!--form:errors path="*" cssClass="text-danger"/!-->
			       ...
			       <div class="form-group">
		                ...
		                <form:select path="insuranceInformation.id" class="form-control" style="cursor:pointer">
		  					<c:forEach items="${loggedInPatient.insuranceRecords}" var="insurance" varStatus="status">
		                        <form:option value="${insurance.id}" label="${insurance.providerName}- ${insurance.insuranceId}"/>
		                    </c:forEach>
		                </form:select>
		            </div>

					<div class="form-group">
					    <label style="padding:5px 0 10px 0">PatientName</label>
					    <div class="form-group">
					    ...
					    <form:input path="patientName" class="form-control" placeholder="Please Enter Full Name!" style="width:100%; padding:8px; border-radius:5%;" />
					</div>

					<input type="submit" value="Add Insurance" class="btn btn-success" style="margin: 10px 0; width: 100%; padding: 10px;"/>
				</form:form>
