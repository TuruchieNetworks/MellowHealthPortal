		    ...
		    <c:if test="${status.index < patientInsuranceCreatedAtList.size() && status.index < patientTotalInsuranceLengthOfCoverages.size()}">
		        ...
		        <c:set var="coverageLength" value="${patientTotalInsuranceLengthOfCoverages[status.index]}" />
		         	<tr style="text-decoration:none; color:aqua"class="table-dark">
		                <td><c:out value="${insurance.patient.id}" /></td>
		                <td>
		                	<a href="/mellowHealth/insuranceRecords/${insurance.id}"style="text-decoration:none; color:aqua">
		                		<c:out value="${insurance.insuranceId} ${coverageLength} Months coverage"/>
		                	</a>
		                </td>
		                <td>
		                	<a href="/mellowHealth/insuranceRecords/${insurance.id}"style="text-decoration:none; color:silk">
		                		<c:out value="${insurance.providerName}"/>
		                	</a>
		            <c:choose>
		                <c:when test="${insurance.startDate != null && insurance.expirationDate != null}">
		                ...
		                <td>
		                	<a href="/mellowHealth/insuranceRecords/${insurance.id}"style="text-decoration:none; 
				                <c:choose>
	               				...
	               				<c:out value="${insurance.startDate}"/>
	               			</a>
	               		</td>
		                <td>
		                	<a href="/mellowHealth/insuranceRecords/${insurance.id}"style="text-decoration:none; 
				                <c:choose>
	               				...
	               				<c:out value="${insurance.expirationDate}"/>
	               			</a>
	               		</td>
		           		<c:choose>
						<c:when test="${insurance.patient.id == loggedInPatient.id}">
				        <td style=" 	
			        		<c:choose>
					            <c:when test="${loggedInPatient.insuranceRecords.size() < 1}">
				             ...
				             " class="d-flex justify-content-around">
								<form action="/mellowHealth/insuranceRecords/editInsuranceRecord/${insurance.id}" method="get" style="margin:0 10px 0 0; ">
								...
								<form action="/mellowHealth/insuranceRecords/deleteInsuranceRecord/${insurance.id}" method="post">
						...
						<td class="table-dark d-flex justify-content-around">
							<a href="/mellowHealth/insuranceRecords/${insurance.id}" style="text-decoration:none; 
								<c:choose>
		               			...
		               			<c:out value="${insurance.providerName}: Coverage Period ${insurance.startDate} - ${insurance.expirationDate}"/>
		               		</a>
		               	</td>
						</c:otherwise>
            			</c:choose>
