...
<title>Update Insurance Record!</title>
</head>
	...
	<div class="container-fluid p-6" style=" text-align:center;">
		<a style="width: 100%; display:block; padding:0px;color:brown;text-decoration:none;" href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id }">
			<h1 style="color:azure;border-bottom:2px solid aliceblue;background:rgba(2.11, 24.5, 121, 0.9);border-radius:5%;font-weight:bold;font-size:22px;padding:10px;font-family:cursive;text-align:center;">
				<c:out value="Make Changes On Your Submitted Insurance Information Today ${currentDateTime}"/> <c:out value="${loggedInPatient.patientFirstName}"/> <c:out value="${loggedInPatient.patientLastName}!"/>
			</h1>
		</a>
			<div class ="btn btn-primary"  style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;padding:5px;">
				<form action="/mellowHealth/insuranceRecords/newInsuranceRecord" class ="btn btn-primary"  style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;text-align:center; padding:5px;background:rgba(1.33, 0.64, 30.60, 0.9);border-radius:7%;">
		...
		<div class="row">
			...
			<div class="col">
				<form:form action="/mellowHealth/process/insuranceRecords/editInsuranceRecord/${insuranceInformation.id}" method="PATCH" modelAttribute="insuranceInformation">
				...
				<!--form:errors path="*" cssClass="text-danger"/!-->
			       <div class="form-group">
					...
					<div class="form-group">
					    <label style="padding:5px 0 10px 0">Insurance ID</label>
					    <div class="form-group">
					    ...
					    <form:input path="insuranceId" class="form-control" placeholder="Please Enter Insurance ID!" style="width:100%; padding:8px; border-radius:5%;" />
					</div>

					<div class="form-group">
					    <label style="padding:5px 0 10px 0">Provider</label>
					    <div class="form-group">
					        <form:errors path="providerName" class="text-danger"/>
					    </div>
					    <div class="form-group" id="selectedPatientDiv" style="font-weight:bold; margin-top: 5px;"></div>
					    <form:input path="providerName" class="form-control" placeholder="Please Enter Providers Name!" style="width:100%; padding:8px; border-radius:5%;" />
					</div>
					...
					<div class="form-group">
					    ...
					    <form:input type="localDate" path="startDate" class="form-control" placeholder="Please Enter Start Date!" style="width:100%; padding:8px; border-radius:5%;" />
					</div>

					<div class="form-group">
					    <label style="padding:5px 0 10px 0">Expiration Date</label>
					    <div class="form-group">
					        <form:errors path="startDate" class="text-danger" />
					    </div>
					    <form:input type="localDate" path="expirationDate" class="form-control" placeholder="Please Enter Systolic BloodPressure Value!" style="width:100%; padding:8px; border-radius:5%;" />
					</div>

					<input type="submit" value="Edit Your ${insuranceInformation.providerName} record" class="btn btn-success" style="margin: 10px 0; width: 100%; padding: 10px;"/>
				</form:form>
				<h1 style="width:100%;">
					<a style=" margin:10px 0;width:100%;display:block; padding:10px" href="/mellowHealth/patientsPortal/patients/${loggedInPatient.id}" class="btn btn-warning">
						<c:out value="CANCEL!"/>
					</a>
				</h1>
			</div>
